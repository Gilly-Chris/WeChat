...
import com.example.wechat.databinding.ContactItemBinding
import com.example.wechat.ui.findUser.mQuery
import java.util.*

class ContactsAdapter(private val itemClickCallback: ItemClickCallback) : ListAdapter<User, ContactsAdapter.UserHolder>(DiffCallbackContacts()),
    Filterable,
    OnQueryTextChange {
            ...
            override fun performFiltering(constraint: CharSequence?): FilterResults {
                ...
                if(charString.isEmpty()) {
                   ...
                   for(user in usersList) {
                       if(user.username?.lowercase(Locale.ENGLISH)?.contains(
                               charString.lowercase(Locale.ENGLISH)
                       )!!){
